...
class attendance_qrrecognition:
    ...
    def qr_rec(self,cv2image,vid):
        ...
        checkQRkey= "SELECT qr_key from qrKeys WHERE subject_id=%s and date=%s"
        ...
        if vid.read()[0]:
            ...
            for barcode in barcodes:
                ...
                checkQRrow = mycursor.fetchone()
                QRkey_check=convertTuple(checkQRrow)
                #print ("THis is check QR: " + str(QRkey_check))
                #print ("This is qr key: " + name[3])
                if (QRkey_check==name[3]):
